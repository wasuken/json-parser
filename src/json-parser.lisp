...
(defun json-core-parse (node)
  (cond ((null node) nil)
		((not (listp node))
		 (varies-with-type node))
		((or (null (car node)) (listp (car node)))
		 ...
		 (if (stringp (car node))
			 (let ((left (car node))
			   ...
			   `( ,(intern (varies-with-type (string-trim '(#\Space) left)) :keyword)
				   ...
				   ,@(if (and (stringp (nth 3 node)) (string= (nth 3 node) ","))
						 (json-core-parse (nthcdr 4 node)))))
			 (json-core-parse (car node))))
		)
  )
